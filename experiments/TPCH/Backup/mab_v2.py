...
def get_table_and_column_details():
    ...
    for table_name in schema:
        ...
        columns[table_name] = [c[0] for c in schema[table_name]]  
...
class MAB:
    ...
    def __init__(self, alpha=1.0, vlambda=0.5, alpha_decay_rate=1.0, creation_time_reduction_factor=1, config_memory_MB=1024, qoi_memory=4, max_indexes_per_table=5, max_index_columns=3, include_cols=False, max_include_columns=3):
        ...
        #print(f"Column info: {self.all_columns}")
    ...
    def extract_candidate_indexes(self, QoIs, verbose):
        ...
        for query_object in QoIs:
            ...
            indexes = extract_query_indexes(query_object,  self.MAX_INDEX_COLUMNS, self.INCLUDE_COLS, max_include_columns=self.MAX_INCLUDE_COLUMNS, dbname=DBNAME, exclude_pk_indexes=True)
