...
def main():
    ...
    max_indexes_per_table = 8 #6 
    max_U = 70 #80
    ibg_max_nodes = 500 #100
    doi_max_nodes = 20 #50
    max_doi_iters_per_node = 200 #100
    ...
    if batch_processing in [1, 2]:
        if not use_simple_cost:
            ...
            wfit = WFIT(max_key_columns=3, include_cols=True, max_include_columns=3, simple_cost=False, enable_stable_partition_locking=enable_stable_partition_locking, max_indexes_per_table=max_indexes_per_table, max_U=max_U, ibg_max_nodes=ibg_max_nodes, doi_max_nodes=doi_max_nodes, max_doi_iters_per_node=max_doi_iters_per_node, normalize_doi=normalize_doi, idxCnt=idxCnt, stateCnt=stateCnt, rand_cnt=rand_cnt, execution_cost_scaling=1e-6,creation_cost_fudge_factor=0e-3) 
            ...
            #wfit = WFIT(max_key_columns=3, include_cols=True, max_include_columns=3, simple_cost=True, enable_stable_partition_locking=True, max_indexes_per_table=8, max_U=100, ibg_max_nodes=100, doi_max_nodes=50, max_doi_iters_per_node=100, normalize_doi=False, idxCnt=40, stateCnt=300, rand_cnt=200, execution_cost_scaling=1e0, creation_cost_fudge_factor=1e2) #creation_cost_fudge_factor=1e-2 
            
            wfit = WFIT(max_key_columns=3, include_cols=True, max_include_columns=3, simple_cost=True, enable_stable_partition_locking=enable_stable_partition_locking, max_indexes_per_table=max_indexes_per_table, max_U=max_U, ibg_max_nodes=ibg_max_nodes, doi_max_nodes=doi_max_nodes, max_doi_iters_per_node=max_doi_iters_per_node, normalize_doi=normalize_doi, idxCnt=idxCnt, stateCnt=stateCnt, rand_cnt=rand_cnt, execution_cost_scaling=1e-6, creation_cost_fudge_factor=5e-6) #creation_cost_fudge_factor=1e-5
